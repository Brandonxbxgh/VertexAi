...
export function ActivityFeed() {
  ...
  useEffect(() => {
    if (!client) return;
    async function loadInitial(c: SupabaseClient) {
      const { data } = await c
        .from("activity_log")
        .select("*")
      ...
      <div className="max-h-[600px] space-y-2 overflow-y-auto">
        {events.length === 0 ? (
          ...
          events.map((e) => (
            <div
              key={e.id}
              className="flex items-start gap-3 rounded-lg border border-zinc-800/50 bg-zinc-950/50 px-4 py-3"
            >
              <span
